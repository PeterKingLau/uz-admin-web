...
function buildFormTemplate(conf: FormConf, child: string, type?: 'file' | 'dialog'): string {
    let labelPosition = ''
...
function buildFromBtns(conf: FormConf, type?: 'file' | 'dialog'): string {
    let str = ''
...
function colWrapper(element: BaseElement, str: string): string {
...
type LayoutBuilder = (element: BaseElement) => string

const layouts: Record<LayoutType, LayoutBuilder> = {
    colFormItem(element) {
...
const tags: Record<string, TagBuilder> = {
    ...
    'el-date-picker': el => {
        ...
        const format = el.format ? `format="${el.format}"` : ''
        const valueFormat = el['value-format'] ? `value-format="${el['value-format']}"` : ''
        const type = el.type && el.type !== 'date' ? `type="${el.type}"` : ''
        const readonly = el.readonly ? 'readonly' : ''
    ...
    'el-rate': el => {
        ...
        return `<${el.tag} ${vModel} ${allowHalf} ${showText} ${showScore} ${max} ${disabled}></${el.tag}>`
    },
    ...
    'el-upload': el => {
        ...
        const name = el.name && el.name !== 'file' ? `name="${el.name}"` : ''
...
function attrBuilder(el: BaseElement): {
    vModel: string
    clearable: string
    placeholder: string
    width: string
    ...
    return {
        vModel: `v-model="${formModel}.${el.vModel}"`,
...
function buildElButtonChild(conf: BaseElement): string {
    ...
    if (conf.default) {
        children.push(String(conf.default))
    }
...
export function makeUpHtml(conf: FormConf, type?: 'file' | 'dialog'): string {
