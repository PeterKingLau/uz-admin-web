<template>
    <div class="app-container">
        ...
        <el-dialog title="内容详情" v-model="openDetail" width="720px" append-to-body>
            <el-descriptions :column="2" border class="mb8">
                ...
                <el-descriptions-item label="帖子类型">
                    <EnumTag enum-type="POST_TYPE" :value="current?.postType" fallback-label="未知" />
                </el-descriptions-item>
...
const columns = computed(() => {
    const enabledKeys = tableColumnStore.getEnabledKeys(CONTENT_AUDIT_TABLE_KEY) || CONTENT_AUDIT_COLUMNS.map(c => c.key)
...
function submitReject() {
    ...
    auditPost({
        ...
        .then(() => {
            ...
            getList()
            if (openDetail.value && current.value && current.value.id === rejectTarget.value.id) {
